...
{    
    ...
    printf("(phrutis modification 08.04.2022)\n\n");
...
void printSpeed(double speed) {
    ...
    iss2 >> ostatok;
    uint64_t sekki = ostatok / 10000000000;
    ...
    if (sekki < 86400) {
        ...
        if (sekki < 31536000) {
            ...
            sprintf(timeStr2, "%0*d days %0*d:%0*d:%0*d", 3, days, 2, hours, 2, minutes, 2, seconds);
            
        }
        else {
            uint64_t input_seconds = sekki;
            uint64_t cseconds_in_year = 31536000;
            uint64_t cseconds_in_day = 86400;
            uint64_t cseconds_in_hour = 3600;
            uint64_t cseconds_in_minute = 60;
            uint64_t cseconds = 1;

            uint64_t years = input_seconds / cseconds_in_year;
            uint64_t days = (input_seconds - (years * 31536000)) / cseconds_in_day;
            uint64_t hours = (input_seconds % cseconds_in_day) / cseconds_in_hour;
            uint64_t minutes = ((input_seconds % cseconds_in_day) % cseconds_in_hour) / cseconds_in_minute;
            uint64_t seconds = (((input_seconds % cseconds_in_day) % cseconds_in_hour) % cseconds_in_minute) / cseconds;

            sprintf(timeStr2, "%0*d years %0*d days %0*d:%0*d:%0*d", 3, years, 3, days, 2, hours, 2, minutes, 2, seconds);
        }
    }
    ...
    if (turbo > 0) {
        verh = " GPU: " + num_gpu + "   C: " + num_str + "   E: " + timeStr2 + "  " + zamena + " -> " + zamena2.c_str();
        printf("\r  [%s] [%s] [S: %s%s] [C: %s%%] [T: %s]  ", toTimeStr(t1, timeStr), wif, formatThousands(speedx).c_str(), valx.c_str(), num_str, formatThousands(vtotal).c_str());
    }
    else {
