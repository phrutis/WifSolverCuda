...
{    
    printf("\n  WifSolverCuda v2.1 Linux (phrutis modification 08.04.2022)\n\n");
...
void printSpeed(double speed) {
    ...
    if (turbo > 0) {
        ...
        printf("\r[%s] [%s] [%s%%] [%s%s] [%s] ", toTimeStr(t1, timeStr), wif, num_str, formatThousands(speedx).c_str(), valx.c_str(), formatThousands(vtotal).c_str());
    }
    else {
        printf("\r[%s] [%s] [%s%%] [%s%s] [%s] ", toTimeStr(t1, timeStr), wif, num_str, formatThousands(speedx).c_str(), valx.c_str(), formatThousands(vtotal).c_str());
    }
...
void printConfig() {
    ...
    if (turbo == 0) {
        ...
        printf("  TURBO MODE      : ON (every 60 sec)\n");
